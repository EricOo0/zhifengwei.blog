...
*/
    ...
    std::cout<<"post_size"<<post_size<<std::endl;
   // sprintf(tmp1, "%s\n", post_data);
    if(post_size >0){	
    	 memcpy(tmp, post_data, post_size);
	tmp1=tmp;
    }
    std::cout<<"post_data:"<<tmp1<<std::endl;
    std::cout<<"post_data_size:"<<tmp1.size()<<std::endl;
    //strcat(output1, tmp);
    /*
    ...
    struct evbuffer *buf;
    buf = evbuffer_new();
    std::ifstream t("html/index_new.html");  
  ...
  // process 
    std::cout.flush();
    t.close();
    evbuffer_add_printf(buf, contents.c_str());
